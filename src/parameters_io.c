    ...
    {           /* input the data from the file. */
        while (fgets(string, 128, fp))
        {
            ...
            if (strcmp(tmp, "nkx") == 0)
            {
                ...
                if (mpi_my_rank == MPI_IO) printf("[MPI process %d] nkx = %zu\n", mpi_my_rank, parameters.nkx);
            }
            if (strcmp(tmp, "nky") == 0)
            {
                sscanf(string, "%*s : %zu", &parameters.nky);
                if (mpi_my_rank == MPI_IO) printf("[MPI process %d] nky = %zu\n", mpi_my_rank, parameters.nky);
            }
            if (strcmp(tmp, "nz") == 0)
            {
                sscanf(string, "%*s : %zu", &parameters.nz);
                if (mpi_my_rank == MPI_IO) printf("[MPI process %d] nz = %zu\n", mpi_my_rank, parameters.nz);
            ...
            if (strcmp(tmp, "nm") == 0)
            {
                ...
                if (mpi_my_rank == MPI_IO) printf("[MPI process %d] nm = %zu\n", mpi_my_rank, parameters.nm);
            }
            if (strcmp(tmp, "nl") == 0)
            {
                sscanf(string, "%*s : %zu", &parameters.nl);
                if (mpi_my_rank == MPI_IO) printf("[MPI process %d] ns = %zu\n", mpi_my_rank, parameters.nl);
            }
            if (strcmp(tmp, "ns") == 0)
            {
            ...
            if (strcmp(tmp, "dealiasing") == 0)
            {
                ...
                {
                    if (mpi_my_rank == MPI_IO) printf("[MPI process %d] dealiasing rule is %d\n", mpi_my_rank, parameters.dealiasing);
            ...
            if (strcmp(tmp, "save_EMfield") == 0)
            {
                ...
                if (mpi_my_rank == MPI_IO) printf("[MPI process %d] save electromagnetic fields = %d \n", mpi_my_rank, parameters.save_EMfield);
            }
        }
    }
    init_global_size();
